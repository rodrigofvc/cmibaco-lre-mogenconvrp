...
def ibaco_indicator(params, pheromone_matrix, indicator, cooperative_mode, execution_n, apply_lns=False, apply_crossover=True, apply_mutation=True, classic=False):
    seed = params['seed']
    random.seed(seed)
    np.random.seed(seed)
    rho = params['rho']
    ...
    for i in range(max_iterations):
        ...
        if not classic:
            ...
            if apply_mutation:
                mutation_stage(current_population, p_mut)
...
def cooperative_ibaco_components(params, n_execution=0, apply_lns=False, apply_crossover=True, apply_mutation=True, classic=False):
    seed = params['seed']
    random.seed(seed)
    np.random.seed(seed)
    matrices = []
    ...
    if apply_crossover or apply_mutation:
        params['n_ants'] = 45
    if classic:
        params['n_ants'] = 90
    ...
    for i in range(max_iterations_cibaco):
        ...
        for j in range(k):
            ...
            P,_,_,_,_,_,all_solutions_p,_ = ibaco_indicator(params, pheromone_matrix, indicator, True, n_execution, apply_lns, apply_crossover, apply_mutation, classic)
